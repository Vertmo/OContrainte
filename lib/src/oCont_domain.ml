...
let card d = match d with
  | Range (start, stop) -> stop - start + 1
  | Arr a -> Array.length a

let add d n = match d with
  | Range (start, stop) when n >= start && n < stop -> d
  | Range (start, _) when n < start -> Arr (Array.of_list (n::(asList d)))
  | Range _ -> Arr (Array.of_list ((asList d)@[n]))
  | Arr a -> Arr (Array.of_list (List.sort_uniq (-) (n::(Array.to_list a))))

let rec remove d n = match d with
  | Range (start, stop) when n < start || n > stop -> d
  | Range (start, stop) -> Arr (Array.of_list ((asList (Range (start,n-1)))@(asList (Range (n+1,stop)))))
