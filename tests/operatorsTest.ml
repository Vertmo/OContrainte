...
let testPropagateSup test_ctxt =
  let dom1 = (range 1 7) and dom2 = fromList [3;4;9] in
  let (dom1, dom2) = (~>).propagate (dom1, dom2) in
  assert_equal (asList dom1) [4;5;6];
  assert_equal (asList dom2) [3;4]

let testPropagateInfEq test_ctxt =
  let dom1 = (range 1 7) and dom2 = fromList [0;3;4] in
  let (dom1, dom2) = (~<=).propagate (dom1, dom2) in
  assert_equal (asList dom1) [1;2;3;4];
  assert_equal (asList dom2) [3;4]

let suite = [
  "propagateEq">::testPropagateEq;
  "propagateSup">::testPropagateSup;
  "propagateInfEq">::testPropagateInfEq;
]
